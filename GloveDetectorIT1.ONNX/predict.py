    ...
    while cam.isOpened():
        ...
        for box, class_id, score in zip(predictions['detected_boxes'][0], predictions['detected_classes'][0], predictions['detected_scores'][0]):
            if score > PROB_THRESHOLD:
                ...
                opencvImage = cv2.rectangle(opencvImage, (x_min, y_min), (x_max, y_max), get_border_color(score), 2)
                
                # Display the class label and confidence score
                label = f"{class_id}: {score:.2f}"
                cv2.putText(opencvImage, label, (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, get_border_color(score), 2)


        opencvImage = cv2.copyMakeBorder(opencvImage, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=(0, 255, 0))
        shapes = np.zeros_like(opencvImage, np.uint8)
        
        cv2.rectangle(shapes, (0, 600), (660, 385), (0, 255, 0), -1)
        ...
        cv2.imshow('Glove Detector IT1', out)
